...
class AppWindow(QMainWindow):
    ...
    def show_cell_data(self, row_no, *args):
        ...
        if self.ui.tableWidget_problems.cellWidget(row_no, 3).currentIndex() == 0:
            self.ui.label_complete_def.setText(
                data_text
                + f'<br>Link: <a href="{self.links[row_no]}">{self.links[row_no]}</a>'
            )
        else:
            self.ui.label_complete_def.setText(
                data_text
    ...
    def save_data(self, mode="save", no_message=False):
        ...
        if mode == "save":
            ...
            if not no_message:
                showMessage(
                    "File Saved", f"Data File saved at <em>{DEFAULT_FILE}</em>", "info"
                )
        elif mode == "save-as":
            ...
            if file_name:
                with open(file_name, "w") as json_file:
                    json.dump(all_data, json_file, indent=3)
                showMessage(
                    "File Saved", f"Data File saved at <em>{file_name}</em>", "info"
                )
